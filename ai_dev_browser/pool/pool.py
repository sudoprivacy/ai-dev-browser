...
class BrowserPool(Generic[ClientT]):
    ...
    Profile modes:
        - "shared" (default): All workers share cookies from a master cookies file.
          First run: login in browser, cookies saved to ~/.ai-dev-browser/cookies.dat
          Subsequent runs: cookies loaded into each worker.
        - "per_worker": Each worker has independent persistent cookies file.
          Worker 0 uses ~/.ai-dev-browser/cookies/cookies_worker_0.dat, etc.
    ...
    Args:
        ...
        cookies_file: Path to shared cookies file (default: ~/.ai-dev-browser/cookies.dat)
        cookies_dir: Base directory for per-worker cookies (default: ~/.ai-dev-browser/cookies)
